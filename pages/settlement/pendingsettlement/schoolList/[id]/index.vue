<script setup>
  ...
  const {data: getSchoolList} = await useFetch("/api/settlement/pendingSettlement/getSchoolList", {
    method: "GET",
    query: {
  ...
  <div>
    ...
    <rs-table v-if="getSchoolList.data && getSchoolList.data.length > 0"
      ...
      <template v-slot:TotalAmount(RM)="data">
        <p> {{ number_format(data.value.sum) }} </p>
      </template>
      ...
      <template v-slot:Action="data">
			<nuxt-link :to="`/settlement/pendingsettlement/${data.value.payment_date}?param=${data.value.school_id}`">
