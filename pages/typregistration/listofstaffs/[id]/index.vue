<script setup>
  ...
  const {data: staffsData} = await useFetch("/api/listofusers/listofstaffs", {
    method: "GET",
    query: {
      school_id: paramID,
    },
  });
  console.log(staffsData.value.data);

  const schoolname = staffsData.value.data.school_name;
  console.log("school name:",schoolname);

  const totalstaffs = staffsData.value.data.total_staffs;
  console.log("total staffs:",totalstaffs);

  const registered = staffsData.value.data.total_register_typ;
  console.log("total register:",registered);

  const unregistered = staffsData.value.data.total_unregister_typ;
...
<template>
  <div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-x-6">
      ...
      <rs-card>
        <div class="summary-1 pt-5 pb-3 px-5 flex items-center gap-4">
          ...
          <div class="flex-1 truncate">
            <span class="block font-semibold text-xl leading-tight"> {{totalstaffs}}</span>
      ...
      <rs-card>
        <div class="summary-1 pt-5 pb-3 px-5 flex items-center gap-4">
          ...
          <div class="flex-1 truncate">
            ...
            <span class="text-base font-semibold text-gray-500"
              >Unregisterd Staffs in toyyibPay</span
            >
          </div>
        </div>
      </rs-card>
    ...
    >
      <rs-table v-if="staffsData.data.infoStaffs && staffsData.data.infoStaffs.length > 0"
        :field="field"
        :data="staffsData.data.infoStaffs"
        :options="{
          variant: 'default',
          striped: true,
          bordered: true,
      ...
      <!-- MPDAL DETAILS -->
      <rs-modal title="Staff Details" position="center" size="lg" v-model="showModalCenter">
        <template #header> 
          <h4>Staff Details</h4> 
        </template>
        ...
        <template #body> 
          <table class="table-auto">
            ...
            <tr>
              <th><p class="text-left">Stafff Type</p></th>
              <td>: {{selectUser.value.staff_type}}</td>
            </tr>
            <tr>
